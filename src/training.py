...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        save_path = f"{config.get('checkpoint_path', 'checkpoints/')}_{model.model_name}_epoch{epoch+1}_trainloss{train_loss:.4f}" + (f"_valloss{val_loss:.4f}" if not only_train else "") + ".pt"
        torch.save({
            "model_state_dict": model.state_dict(),
            "backbone": model.backbone.state_dict(),
            "classifier": model.classifier.state_dict(),
